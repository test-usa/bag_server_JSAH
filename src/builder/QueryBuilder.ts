import { FilterQuery, Query } from "mongoose";
import { excludedFields } from "../constent";

class QueryBuilder<T> {
  // <T> it works for different type of data
  public modelQuery: Query<T[], T>;
  public query: Record<string, unknown>;

  constructor(modelQuery: Query<T[], T>, query: Record<string, unknown>) {
    this.modelQuery = modelQuery; // assign the value to the constructer
    this.query = query;
  }

  search(searchableFields: string[]) {
    const searchTerm = this?.query?.searchTerm;

    console.log(searchTerm);

    if (typeof searchTerm === "string" && searchTerm.trim()) {
      const terms = searchTerm.split(",").map((term) => term.trim()); // Ensure searchTerm is a valid string

      this.modelQuery = this.modelQuery.find({
        $or: terms.flatMap((term) =>
          searchableFields.map((field) => ({
            [field]: { $regex: term, $options: "i" },
          }))
        ),
      });
    }

    return this;
  }

  filter() {
    const queryObject = { ...this.query };
    const exclude = excludedFields;
    exclude.forEach((ele) => delete queryObject[ele]);
    this.modelQuery = this.modelQuery.find(queryObject as FilterQuery<T>);

    return this;
  }
  mostPopularProducts() {
    const popularItem = this.query.popularItem;
    console.log(this.query);

    if (popularItem) {
      const item = Number(popularItem); // Convert to number
      console.log(item);

      if (!isNaN(item)) {
        // Check if the conversion was successful
        this.modelQuery = this.modelQuery
          .sort({ sellsQuantity: -1 })
          .limit(item);
      }
    }

    return this;
  }

  sort() {
    const sort =
      (this?.query?.sort as string)?.split(",")?.join(" ") || "createdAt";
    this.modelQuery = this.modelQuery.sort(sort as string);
    return this;
  }

  paginate() {
    const page = Number(this?.query?.page) || 1;
    const limit = Number(this?.query?.limit) || 10;
    const skip = (page - 1) * limit;

    // console.log(page, limit);

    this.modelQuery = this.modelQuery.skip(skip).limit(limit);

    return this;
  }

  fields() {
    const fields =
      (this?.query?.fields as string)?.split(",")?.join(" ") || "-__V";

    this.modelQuery = this.modelQuery.select(fields);
    return this;
  }

  productWithPriceRange() {
    const maxPrice = this.query.price as string;
    if (maxPrice) {
      const price = Number(maxPrice);

      if (!isNaN(price) && price > 0) {
        this.modelQuery = this.modelQuery.find({
          currentPrice: { $gt: 0, $lte: price },
        });
      }
    }

    return this;
  }
  checkInStock() {
    const stock = Boolean(this.query.inStock);
    if (stock) {
      this.modelQuery = this.modelQuery.find({
        totalQuantity: { $gt: 0 },
      });
    } else {
      this.modelQuery = this.modelQuery.find({
        totalQuantity: { $eq: 0 },
      });
    }

    return this;
  }

  //   async countTotal() {
  //     const totalQueries = this.modelQuery.getQuery();
  //     const total = await this.modelQuery.model.countDocuments(totalQueries);
  //     const page = Number(this?.query?.page) || 1;
  //     const limit = Number(this?.query?.limit) || 10;
  //     const totalPage = Math.ceil(total / limit);

  //     return {
  //       page,
  //       limit,
  //       totalPage,
  //     };
  //   }
}

export default QueryBuilder;